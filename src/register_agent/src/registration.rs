...
use evm_adapter::constant::{MINTER_ADDRESS, REGISTRATION_FEE};
use ic_agent::Agent;
...
use crate::constant::{
    ...
    METHOD_REGISTER_IC_AGENT, METHOD_VERIFY_REGISTRATION, NETWORK_IC,
};
...
impl<'a> RegistrationService<'a> {
    ...
    async fn registration_transaction(&self) -> Result<ethers_core::types::Transaction> {
        ...
        info!("creating registration transaction (from: {address}, to: {to}, value: {REGISTRATION_FEE}, nonce: {nonce}, gas_price: 0, gas: 53000)");
