...
use super::params::Params as AlloyParams;
...
impl ParamsAccessors for Request<AlloyParams> {
    fn get_from_vec<T: DeserializeOwned>(&self, index: usize) -> Result<T, ErrorPayload> {
        let AlloyParams::Array(params) = &self.params else {
    ...
    fn get_from_object<T: DeserializeOwned>(&self, field: &str) -> Result<Option<T>, ErrorPayload> {
        let AlloyParams::Map(params) = &self.params else {
...
mod tests {
    ...
    use alloy::rpc::json_rpc::RequestMeta;
