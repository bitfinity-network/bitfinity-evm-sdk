...
pub trait DatabaseClient: Send + Sync {
    /// Initialize the database
     fn init(&self, block: Option<Block<H256>>, reset_database: bool) -> impl Future<Output = anyhow::Result<()>> + Send;
    ...
    /// Returns whether the block hash corresponds to the one in the db
     fn check_if_same_block_hash(&self, block: &Block<H256>) -> impl Future<Output = anyhow::Result<bool>> + Send {
        async {
    ...
    /// Get a block from the database
     fn get_block_by_number(&self, block_number: u64) -> impl Future<Output = anyhow::Result<Block<H256>>> + Send;
    ...
    /// Insert certified block data
     fn insert_certified_block_data(&self, response: CertifiedBlock) -> impl Future<Output = anyhow::Result<()>> + Send;

    /// Returns certified response for the last block
     fn get_last_certified_block_data(&self) -> impl Future<Output = anyhow::Result<CertifiedBlock>> + Send;

    /// Get genesis balances
     fn get_genesis_balances(&self) -> impl Future<Output = anyhow::Result<Option<Vec<AccountBalance>>>> + Send;
    ...
    /// Get a transaction from the database
     fn get_transaction(&self, tx_hash: H256) -> impl Future<Output = anyhow::Result<Transaction>> + Send;
    ...
    /// the given 'reason' and timestamp.
     fn discard_blocks_from(&self, start_from: u64, reason: &str) -> impl Future<Output = anyhow::Result<()>> + Send;

    /// Returns a discarded block by its hash.
     fn get_discarded_block_by_hash(&self, block_hash: H256)
    ...
    fn get_block_info(&self) -> impl Future<Output = anyhow::Result<Option<BlockchainBlockInfo>>> + Send;

    /// Stores blockchain block info.
    fn set_block_info(&self, info: BlockchainBlockInfo) -> impl Future<Output = anyhow::Result<()>> + Send;
}
