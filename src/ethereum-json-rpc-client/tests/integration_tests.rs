...
async fn should_get_block_number() {
    let result = get_block_number(ETHEREUM_JSON_API_URL).await.unwrap();
    assert!(result > 16896634);
}

#[tokio::test]
async fn should_get_block_by_number() {
    let result = get_block_by_number(ETHEREUM_JSON_API_URL, BlockNumber::Number(11588465.into()))
        .await
        .unwrap();
...
async fn should_get_full_block_by_number() {
    let result =
        get_full_block_by_number(ETHEREUM_JSON_API_URL, BlockNumber::Number(11588465.into()))
            .await
            .unwrap();
...
async fn should_get_full_blocks_by_number() {
    let result = get_full_blocks_by_number(
...
async fn should_get_transaction_receipts() {
    let block = get_block_by_number(ETHEREUM_JSON_API_URL, BlockNumber::Number(11588465.into()))
        .await
        .unwrap();
